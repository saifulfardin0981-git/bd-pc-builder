...
# --- HELPER: HOVER BADGE RENDERER ---
def render_power_badge(breakdown):
    css = """
    <style>
    .power-container { position: relative; display: inline-block; cursor: pointer; font-family: sans-serif; }
    .power-badge { background-color: #FF4B4B; color: white; padding: 8px 16px; border-radius: 8px; font-weight: bold; font-size: 18px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); transition: transform 0.2s; }
    .power-badge:hover { transform: scale(1.05); }
    .power-tooltip { visibility: hidden; width: 220px; background-color: #262730; color: #fff; text-align: left; border-radius: 8px; padding: 12px; position: absolute; z-index: 10; top: 125%; left: 50%; margin-left: -110px; box-shadow: 0 8px 16px rgba(0,0,0,0.2); opacity: 0; transition: opacity 0.3s; border: 1px solid #444; }
    .power-container:hover .power-tooltip { visibility: visible; opacity: 1; }
    .power-row { display: flex; justify-content: space-between; margin-bottom: 4px; font-size: 14px; }
    .power-total { border-top: 1px solid #666; padding-top: 8px; margin-top: 8px; font-weight: bold; color: #FF4B4B; }
    </style>
    """
    html = f"""
    {css}
...
# --- UI START ---
st.title("🖥️ BD PC Builder AI v5.1")
...
budget_input = st.number_input("💰 What is your Budget (BDT)?", min_value=15000, max_value=500000, step=1000, value=safe_budget, key="budget_v5")
...
if st.session_state.build_results:
    ...
    if parts:
        ...
        # --- NEW: COPY TEXT FEATURE ---
        with st.expander("📋 View & Copy Parts List"):
            summary_text = generate_build_summary(parts, data['total'], breakdown['Total'])
        ...
        if data["saved"] > 0: st.warning(f"💵 Unused Budget: {data['saved']} BDT")